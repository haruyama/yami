...
(define
  ...
  '(
    ...
    (assign continue (label ev-application-exp-actual))
    (goto (label actual-value))

    ev-application-exp-actual
    ...
    (goto (label actual-value)) ;ex5.25
    ...
    (test (op primitive-procedure?) (reg proc))
    (branch (label ev-appl-operand-loop-actual))
    (test (op compound-procedure?) (reg proc))
    (branch (label ev-appl-operand-loop-delay))
    (goto (label unknown-procedure-type))

    ev-appl-operand-loop-actual
    (save argl)
    (assign exp (op first-operand) (reg unev))
    (test (op last-operand?) (reg unev))
    (branch (label ev-appl-last-arg-actual))
    (save env)
    (save unev)
