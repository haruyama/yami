...
(define (is-not-argl-safe-for-proc? proc argl)
;  (display proc)
;  (newline)
;  (display argl)
;  (newline)
  (let ((impl (primitive-implementation proc)))
    (cond ((eq? impl /)
           (cond 
             ((= 0 (length argl))
              (display "ERROR: procedure requires at least one argument: (/)")
              (newline)
              #t)
             ((= 0 (last-value argl))
              (display "ERROR: attempt to calculate a division by zero")
              (newline)
              #t)
             (else #f))
           )
...
(is-not-argl-safe-for-proc? (list 'primitive /) '())
(is-not-argl-safe-for-proc? (list 'primitive /) '())
(is-not-argl-safe-for-proc? (list 'primitive /) '())
...
(define eceval-operations
  (list
    ...
    (list 'is-not-argl-safe-for-proc?  is-not-argl-safe-for-proc?)
    )
  )
...
(define
  ...
  '(
    ...
    primitive-apply
    (test (op is-not-argl-safe-for-proc?) (reg proc) (reg argl))
    (branch (label not-safe-argl-for-proc))
