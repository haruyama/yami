...
(define (display-deque deque)
  (define (display-deque-internal q display-space)
    (cond ((eq? q (rear-ptr deque))
           (if display-space (display " "))
           (display (value-of-item q)))
          (else
            (begin (if display-space (display " "))
                   (display (value-of-item q))
                   (display-deque-internal (next-item q) #t)))))
  (if (empty-deque? deque)
      (display "empty deque\n")
      (begin
        (display "(")
        (display-deque-internal (front-ptr deque) #f)
        (display ")\n"))))
